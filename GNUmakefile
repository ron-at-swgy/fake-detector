...
# Apple's headers pick a visibility level from the feature macros:
# _POSIX_C_SOURCE alone hides non-POSIX declarations (mkdtemp), and
# under -std=c11 (__STRICT_ANSI__) NO macro at all drops to ISO-C-only
# visibility, hiding even clock_gettime. Ask for POSIX plus Apple's
# extension-restoring switch.
APP_POSIX  += -D_DARWIN_C_SOURCE
else
